...
LinearModelPredictiveControllerNode::LinearModelPredictiveControllerNode(
  ...
  control_timer_ = nh_.createTimer(ros::Duration(1.0 / control_rate_),
                                   &LinearModelPredictiveControllerNode::ControlTimerCallback,
                                   this);
...
void LinearModelPredictiveControllerNode::TrySetOffboard(const ros::Time& now) {
  ...
  if (set_mode_client_.call(set_mode_srv) && set_mode_srv.response.mode_sent) {
    ROS_INFO_THROTTLE(2.0, "Linear MPC node requested OFFBOARD mode.");
  } else {
    ROS_WARN_THROTTLE(2.0, "Linear MPC node failed to request OFFBOARD mode.");
  }
  last_mode_request_ = now;
}

void LinearModelPredictiveControllerNode::TryArm(const ros::Time& now) {
  ...
  if (arming_client_.call(arm_srv) && arm_srv.response.success) {
    ROS_INFO_THROTTLE(2.0, "Linear MPC node requested arming.");
  } else {
...
void LinearModelPredictiveControllerNode::GenerateTakeoffTrajectory() {
  if (!has_odometry_) {
...
bool LinearModelPredictiveControllerNode::IsTakeoffReached() const {
...
void LinearModelPredictiveControllerNode::ControlTimerCallback(const ros::TimerEvent&) {
  const ros::Time now = ros::Time::now();

  if (!has_odometry_) {
    ROS_WARN_THROTTLE(1.0, "Linear MPC node waiting for odometry.");
    return;
  }
...
int main(int argc, char** argv) {
  ...
  mav_control::LinearModelPredictiveControllerNode node(nh, private_nh);
  ros::spin();
  return 0;
}
