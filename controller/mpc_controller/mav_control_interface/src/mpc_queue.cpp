                                           ...
                                           mav_msgs::EigenTrajectoryPointDeque& interpolated_queue)
{
  if(input_queue.size() < 2){
    ...
    // For single-point references (common in streaming setpoint modes),
    // build a short horizon using constant acceleration extrapolation.
    // This provides MPC with preview and reduces lag, especially on Z-axis
    // during arc/inverted-U trajectories.
    ...
    ROS_WARN_THROTTLE(1.0,
                      "MPCQueue: Single-point reference detected, using kinematic extrapolation horizon (%d points).",
                      horizon_points);
    return;
  }
...
//  //fill time_output vector
  ...
  for (auto it = time_output.begin(); it != time_output.end(); ++it) {
    ...
    const double yaw_delta = shortestAngularDistance(yaw_1, yaw_2);

    point.setFromYaw(yaw_1 + yaw_delta * ratio);
