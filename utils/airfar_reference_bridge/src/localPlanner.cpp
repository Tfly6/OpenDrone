...
double collisionCheckPadding = 0.5;
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("collisionCheckPadding", collisionCheckPadding);
  ...
  const std::vector<double> configuredPathScales =
      airfar_reference_bridge::BuildPathScaleSchedule(
          pathScale, minPathScale, pathScaleStep, false, 1.0);
  if (configuredPathScales.empty()) {
    ROS_FATAL_STREAM("Invalid path-scale configuration: pathScale="
                     << pathScale << ", minPathScale=" << minPathScale
                     << ", pathScaleStep=" << pathScaleStep
                     << ". Require pathScale >= minPathScale > 0 and "
                        "pathScaleStep > 0.");
    return 1;
  }
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      *laserCloudStack[laserCloudCount] = *laserCloudDwz;
      ...
      std::vector<double> candidatePathScales;
      if (manualMode || (autonomyMode && autoAdjustMode)) {
        candidatePathScales.push_back(minPathScale);
      } else {
        candidatePathScales =
            airfar_reference_bridge::BuildPathScaleSchedule(
                pathScale, minPathScale, pathScaleStep, pathScaleBySpeed,
                joyFwd);
      }

      for (std::size_t scaleIndex = 0;
           scaleIndex < candidatePathScales.size(); ++scaleIndex) {
        const float activePathScale = candidatePathScales[scaleIndex];
        for (int i = 0; i < pathNum; i++) {
          clearPathList[i] = 0;
        }
        for (int i = 0; i < groupNum; i++) {
        ...
        for (int i = 0; i < plannerCloudSize; i++) {
          float x = plannerCloud->points[i].x / activePathScale;
          float y = plannerCloud->points[i].y / activePathScale;
          float z = plannerCloud->points[i].z / activePathScale;
          float dis = sqrt(x * x + y * y);

          if (x > 0 && (dis <= (relativeGoalDis + collisionCheckPadding) / activePathScale || relativeGoalX < 0) &&
              z > lowerBoundZ / activePathScale && z < upperBoundZ / activePathScale) {
        ...
        if (selectedGroupID >= 0) {
          ...
          for (int i = 0; i < selectedPathLength; i++) {
            ...
            if (dis <= relativeGoalDis / activePathScale || relativeGoalX < 0) {
              path.poses[i].pose.position.x = activePathScale * x;
              path.poses[i].pose.position.y = activePathScale * y;
              path.poses[i].pose.position.z = activePathScale * z;
            } else {
              path.poses.resize(i);
              break;
            }
          ...
          for (int i = 0; i < pathNum; i++) {
            if (clearPathList[i] < pointPerPathThre) {
              ...
              for (int j = 0; j < freePathLength; j++) {
                ...
                if (dis <= (relativeGoalDis + collisionCheckPadding) / activePathScale || relativeGoalX < 0) {
                  point.x *= activePathScale;
                  point.y *= activePathScale;
                  point.z *= activePathScale;
      ...
      if (!pathPublished) {
        if (!candidatePathScales.empty()) {
          ROS_WARN_THROTTLE(
              1.0,
              "No collision-free path at any configured scale "
              "(first %.3f, minimum %.3f, %zu attempts, %d cloud points)",
              candidatePathScales.front(), candidatePathScales.back(),
              candidatePathScales.size(), plannerCloudSize);
        } else {
          ROS_ERROR_THROTTLE(
              1.0,
              "Cannot build path-scale schedule from speed ratio %.3f",
              joyFwd);
        }
        path.poses.resize(1);
