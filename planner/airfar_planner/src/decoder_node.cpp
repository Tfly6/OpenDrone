...
void GraphDecoder::Init() {
    ...
    graph_pub_     = nh.advertise<airfar_planner::VisGraph>("decoded_vgraph", 5);
...
void GraphDecoder::GraphCallBack(const airfar_planner::VisGraphConstPtr& msg) {
    const airfar_planner::VisGraph shared_graph = *msg;
    this->ResetGraph(received_graph_);
...
void GraphDecoder::CreateNavNode(const airfar_planner::VisNode& msg,
                                 NavNodePtr& node_ptr)
{
...
void GraphDecoder::EncodeGraph(const NodePtrStack& graphIn, airfar_planner::VisGraph& graphOut) {
    graphOut.nodes.clear();
    const std::string frame_id = graphOut.header.frame_id;
    for (const auto& node_ptr : graphIn) {
        airfar_planner::VisNode msg_node;
...
void GraphDecoder::ReadGraphCallBack(const std_msgs::StringConstPtr& msg) {
    ...
    airfar_planner::VisGraph graph_msg;
