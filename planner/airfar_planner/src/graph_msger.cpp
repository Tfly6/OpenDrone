...
void GraphMsger::Init(const ros::NodeHandle& nh, const GraphMsgerParams& params) {
    nh_ = nh;
    gm_params_ = params;
    graph_pub_ = nh_.advertise<airfar_planner::NavGraph>("/planner_nav_graph", 5);
...
void GraphMsger::EncodeGraph(const NodePtrStack& graphIn, airfar_planner::NavGraph& graphOut) {
    ...
    for (const auto& node_ptr : graphIn) {
        ...
        airfar_planner::NavNode msg_node;
